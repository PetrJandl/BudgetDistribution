                    ...
                    >
                        <li class="nav-link d-block d-sm-none">
                            <a
                                ...
                                @click="navigate"
                                :title="
                                    countPiece + ' ks za ' + sumPrice + 'Kč'
                                "
                            >
                                <i class="fa">&#xf07a;</i>
                                <span
                                    class="badge badge-warning bg-danger"
                                    id="lblCartCount"
                                >
                                    {{ sumPrice }}Kč
                                </span>
...
export default {
    ...
    methods: {
            ...
            if (this.sumPrice + $newItem.price * $newItem.piece < maxPriceSum) {
                ...
                if (update == 0) {
                    ...
                    this.basked.forEach((item) => {
                        if (item.iditem == $newItem.iditem) {
                            item.pieceInBasket =
                                item.pieceInBasket + $newItem.piece;
                            this.sumPrice = item;
                        }
                    });
                }
                ...
                alert(
                    "Celková částka objednávky nesmí překročit " + maxPriceSum
                );
            }
        },
        ...
        updateBasked: function ($updateItem) {
            ...
            if (
                sum + $updateItem.price * $updateItem.pieceInBasket <
                maxPriceSum
            ) {
                ...
                this.basked[this.basked.indexOf($updateItem)].pieceInBasket =
                    this.basked[this.basked.indexOf($updateItem)]
                        .pieceInBasket - 1;
                alert(
                    "Celková částka objednávky nemůže překročit " + maxPriceSum
                );
                this.backdoor++;
            }
        },
